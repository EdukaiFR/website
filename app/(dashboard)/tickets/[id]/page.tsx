...
export default function TicketDetailPage() {
    ...
    const [commentVisibility, setCommentVisibility] = useState<CommentVisibilityEnum>(CommentVisibilityEnum.PUBLIC);
    ...
    const isSupport = (role: string) => {
        return role.toLowerCase().includes('support') || role.toLowerCase().includes('admin') || role.toLowerCase().includes('agent');
    };
    ...
    const getUserAvatar = (name: string, role: string) => {
        ...
        return {
            initials,
            bgColor: isSupportUser ? 'bg-gradient-to-r from-blue-500 to-indigo-500' : 'bg-gradient-to-r from-gray-400 to-gray-500',
                    ...
                    <div className="relative z-10 px-4 sm:px-6 py-8 sm:py-12 max-w-6xl mx-auto">
                        <Card className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-blue-100/50 hover:shadow-2xl hover:scale-[1.02] transition-all duration-300">
                            <CardContent className="p-12 text-center">
                                ...
                                <p className="text-gray-600 mb-8">Le ticket demandé n&apos;existe pas ou n&apos;est plus accessible.</p>
                                <Button 
    ...
    const canReopen = currentTicket.status === TicketStatus.CLOSED || currentTicket.status === TicketStatus.RESOLVED;
                ...
                <div className="relative z-10 px-4 sm:px-6 py-8 sm:py-12 max-w-6xl mx-auto space-y-6 sm:space-y-8">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                        ...
                        {canReopen && (
                            ...
                            >
                                <RefreshCw className={cn("w-5 h-5 mr-2", isReopening && "animate-spin")} />
                                Rouvrir le ticket
                            </Button>
                        )}
                    </div>
                            ...
                            <div className="flex-1">
                                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                                    ...
                                    <div className={cn(
                                        ...
                                        currentTicket.status === TicketStatus.NEW && "bg-blue-50/90 border-blue-200/50 text-blue-700",
                                        currentTicket.status === TicketStatus.IN_PROGRESS && "bg-amber-50/90 border-amber-200/50 text-amber-700",
                                        currentTicket.status === TicketStatus.RESOLVED && "bg-green-50/90 border-green-200/50 text-green-700",
                                        currentTicket.status === TicketStatus.CLOSED && "bg-gray-50/90 border-gray-200/50 text-gray-700",
                                        currentTicket.status === TicketStatus.REOPENED && "bg-orange-50/90 border-orange-200/50 text-orange-700"
                                    )}>
                                    ...
                                    <div className={cn(
                                        ...
                                        currentTicket.priority === TicketPriority.P0 && "bg-red-500 text-white shadow-red-200 shadow-lg",
                                        currentTicket.priority === TicketPriority.P1 && "bg-orange-500 text-white shadow-orange-200 shadow-lg",
                                        currentTicket.priority === TicketPriority.P2 && "bg-blue-500 text-white shadow-blue-200 shadow-lg",
                                        currentTicket.priority === TicketPriority.P3 && "bg-gray-500 text-white shadow-gray-200 shadow-lg"
                                    )}>
                                        ...
                                        <CardContent className="p-4 sm:p-6 space-y-3 sm:space-y-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="font-semibold text-gray-900">{currentTicket.reporter.name}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="font-semibold text-gray-900">
                                                        {new Date(currentTicket.createdAt).toLocaleDateString('fr-FR', {
                                                            weekday: 'long',
                                                            year: 'numeric',
                                                            month: 'long',
                                                            day: 'numeric'
                                            ...
                                            {currentTicket.updatedAt !== currentTicket.createdAt && (
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="font-semibold text-gray-900">
                                                            {new Date(currentTicket.updatedAt).toLocaleDateString('fr-FR', {
                                                                weekday: 'long',
                                                                year: 'numeric',
                                                                month: 'long',
                                                                day: 'numeric'
                                        ...
                                        <CardContent className="p-4 sm:p-6 space-y-3 sm:space-y-4">
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-between">
                                                    <p className="text-sm text-gray-600 font-medium">URL de la page</p>
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        onClick={() => handleCopyUrl(currentTicket.context.pageUrl)}
                                            ...
                                            {currentTicket.context.viewport && (
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-sm text-gray-600">Résolution écran</p>
                                                        <p className="font-semibold text-gray-900">
                                                            {currentTicket.context.viewport.w} × {currentTicket.context.viewport.h} pixels
                                                        </p>
                                                    </div>
                                                </div>
                                            )}
                                    ...
                                    <Card className="bg-orange-50/50 backdrop-blur-sm hover:bg-orange-50/80 hover:shadow-lg transition-all duration-300 border border-orange-100/50">
                                        <CardContent className="p-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-semibold text-gray-900">
                                                        {currentTicket.attachments.length} fichier{currentTicket.attachments.length > 1 ? 's' : ''} joint{currentTicket.attachments.length > 1 ? 's' : ''}
                                                    </p>
                        ...
                        <CardContent className="p-4 sm:p-6 lg:p-8">
                            ...
                            {comments.length === 0 ? (
                                <div className="text-center py-16">
                                    ...
                                    <h3 className="text-xl font-semibold text-gray-900 mb-2">Aucun message pour le moment</h3>
                                    <p className="text-gray-500">Commencez la conversation en ajoutant votre premier commentaire</p>
                                </div>
                            ) : (
                                <div className="space-y-3">
                                    {comments
                                        .filter(comment => {
                                            ...
                                            if (permissions.canSeeInternalComments) {
                                                return true;
                                            }
                                            // Regular users can only see public comments
                                            return comment.visibility === CommentVisibilityEnum.PUBLIC;
                                        })
                                        .sort((a, b) => new Date(a.at).getTime() - new Date(b.at).getTime())
                                        .map((comment, index, sortedComments) => {
                                        const isSupportUser = isSupport(comment.author.role);
                                        const avatar = getUserAvatar(comment.author.name, comment.author.role);
                                        const isConsecutive = index > 0 && sortedComments[index - 1].author.name === comment.author.name;
                                        const showTimestamp = !isConsecutive;
                                        ...
                                        return (
                                            <div key={comment._id}>
                                                    ...
                                                    <div className="flex items-center justify-center mb-4 mt-6 first:mt-0">
                                                        <div className="flex items-center gap-3 bg-gray-100/80 rounded-full px-4 py-2">
                                                            ...
                                                            <span className="text-xs text-gray-500">
                                                                {new Date(comment.at).toLocaleString('fr-FR', {
                                                                    day: 'numeric',
                                                                    month: 'short',
                                                                    hour: '2-digit',
                                                                    minute: '2-digit'
                                                ...
                                                <div className={cn(
                                                    "flex mb-1",
                                                    isSupportUser ? "justify-start" : "justify-end"
                                                )}>
                                                    <div className={cn(
                                                        "max-w-[70%] sm:max-w-xs md:max-w-md lg:max-w-lg xl:max-w-xl rounded-3xl px-3 sm:px-4 py-2 sm:py-3 relative",
                                                        comment.visibility === CommentVisibilityEnum.INTERNAL && "border-2 border-amber-400 bg-amber-50",
                                                        comment.visibility === CommentVisibilityEnum.PUBLIC && isSupportUser && "bg-gray-200 text-gray-900 rounded-bl-lg",
                                                        comment.visibility === CommentVisibilityEnum.PUBLIC && !isSupportUser && "bg-blue-500 text-white rounded-br-lg",
                                                        comment.visibility === CommentVisibilityEnum.INTERNAL && isSupportUser && "bg-amber-100 text-amber-900 rounded-bl-lg border-amber-300",
                                                        comment.visibility === CommentVisibilityEnum.INTERNAL && !isSupportUser && "bg-amber-200 text-amber-900 rounded-br-lg border-amber-300",
                                                        "shadow-sm"
                                                    )}>
                                                        ...
                                                        {isConsecutive && (
                                                            <div className={cn(
                                                                ...
                                                                isSupportUser ? "text-gray-600" : "text-blue-100"
                                                            )}>
                                                                {new Date(comment.at).toLocaleString('fr-FR', {
                                                                    hour: '2-digit',
                                                                    minute: '2-digit'
                                ...
                                <div className="bg-gradient-to-r from-gray-50 to-blue-50/30 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-100/50">
                                    <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500">Ajoutez votre commentaire à la conversation</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {permissions.canCreateInternalComments && (
                                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 mt-3">
                                                ...
                                                <Select 
                                                    ...
                                                    onValueChange={(value) => setCommentVisibility(value as CommentVisibilityEnum)}
                                                >
                                                    <SelectTrigger className="w-full sm:w-[200px] h-8">
                                                        <SelectValue />
                                                    </SelectTrigger>
                                                    <SelectContent>
                                                        <SelectItem value={CommentVisibilityEnum.PUBLIC}>
                                                            🌐 Public - Visible par tous
                                                        </SelectItem>
                                                        <SelectItem value={CommentVisibilityEnum.INTERNAL}>
                                        ...
                                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 mt-3">
                                            <div className="text-xs text-gray-500">
                                                {newComment.length > 0 && (
                                                    <span>{newComment.length} caractères</span>
                                                )}
                                            </div>
                                            <Button
                                            ...
                                            >
                                                <Send className={cn(
                                                    "w-5 h-5 mr-2",
                                                    isAddingComment && "animate-pulse"
                                                )} />
                                                {isAddingComment ? "Envoi en cours..." : "Envoyer le message"}
                                            </Button>
                                        </div>
                                    </div>
                                </div>
