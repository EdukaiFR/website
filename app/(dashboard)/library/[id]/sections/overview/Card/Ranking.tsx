...
export const Ranking = ({ ranking, className }: RankingProps) => {
  ...
  if (!typedRanking || typedRanking.length === 0) {
    return (
      <div className={`bg-white/70 backdrop-blur-sm rounded-2xl p-4 flex flex-col h-full border-0 shadow-lg hover:shadow-xl transition-all duration-200 ${className}`}>
        {/* Header */}
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl">
            <Crown className="w-4 h-4 text-white" />
          </div>
          <h3 className="text-base font-semibold text-gray-800">Classement</h3>
        </div>
        ...
        <div className="flex flex-col items-center justify-center gap-3 h-full text-center">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Lance des quiz pour voir ton classement apparaître.
            </p>
          </div>
        </div>
      </div>
