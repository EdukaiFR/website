...
export const Statistics = ({
  ...
  useEffect(() => {
    const fetchInsights = async () => {
      if (quiz_id && insights_service && !hasAttemptedFetch) {
        console.log("🔍 [Statistics] Fetching insights for quiz:", quiz_id);
        setLoading(true);
        ...
        try {
          ...
          const dataPromise = insights_service.getQuizInsights(quiz_id);
          const data = await Promise.race([dataPromise, timeoutPromise]);

          console.log("✅ [Statistics] Insights fetched:", data);
          setLocalInsights(data);
        } catch (error) {
          console.error("❌ [Statistics] Failed to fetch insights:", error);
  ...
  useEffect(() => {
    if (!quiz_id || !insights_service || !hasAttemptedFetch || loading) return;
  ...
  const calculateStats = (data = currentInsights) => {
    ...
    if (data.insights && data.insights.length > 0) {
      ...
      if (insights.length >= 2) {
        if (insights.length >= 4) {
          ...
          const previous =
            insights.slice(-4, -2).reduce((a, b) => a + b.score, 0) / 2;
      ...
      const recentPerformance = insights.slice(-10).map((insight, index) => ({
    ...
    // No data at all
    console.log("⚠️ [Statistics] No insights data available - returning zeros");
    return {
      totalQuizzes: 0,
      averageScore: 0,
      highestScore: 0,
  ...
  if (!hasData) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-center">
        ...
        <p className="text-gray-500 text-sm max-w-md">
          Complétez quelques quiz pour voir vos statistiques détaillées et
  ...
  const processInsightsData = (data: any) => {
    if (data.insights && data.insights.length > 0) {
      ...
      console.log("✅ [Statistics] Using real insights data:", data.insights);
      return data;
    } else if (data.insightsCount && data.insightsCount > 0) {
      ...
      return {
        ...
        insightsCount: parseInt(data.insightsCount) || 0,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500">Total des tentatives</p>
          </CardContent>
        </Card>
            ...
            {hasRealInsights && (
              <div className="flex items-center space-x-1 text-xs text-gray-500">
                ...
                <span>
                  ...
                  {Math.round(stats.improvementTrend)}% vs précédent
                </span>
              </div>
            )}
          </CardContent>
          ...
          <>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-gray-500">Quiz réussis (≥70%)</p>
              </CardContent>
            </Card>
          </>
        )}
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-gray-500">
                Complétez plus de quiz pour débloquer les graphiques détaillés,
                l'évolution des performances et l'analyse des tendances.
              </p>
            </div>
          </CardContent>
        </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                ...
                <Progress
                  value={
                    (stats.scoreDistribution.excellent / stats.totalQuizzes) *
                    100
                  }
                  className="h-2"
                />
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm">Bien (70-89%)</span>
                  </div>
                  <span className="font-semibold">
                ...
                <Progress
                  value={
                    (stats.scoreDistribution.good / stats.totalQuizzes) * 100
                  }
                  className="h-2"
                />
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm">Moyen (50-69%)</span>
                  </div>
                  <span className="font-semibold">
                ...
                <Progress
                  value={
                    (stats.scoreDistribution.average / stats.totalQuizzes) * 100
                  }
                  className="h-2"
                />
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm">À améliorer (&lt;50%)</span>
                  </div>
                  <span className="font-semibold">
                ...
                <Progress
                  value={
                    (stats.scoreDistribution.needsWork / stats.totalQuizzes) *
                    100
                  }
                  className="h-2"
                />
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg border border-green-200">
                ...
                <div className="text-sm text-gray-600">Quiz réussis (≥70%)</div>
              </div>
            </div>
          </CardContent>
        </Card>
