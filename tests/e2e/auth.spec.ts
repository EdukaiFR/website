...
test.describe('Authentication Flow', () => {
  test('should display login page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Edukai/i })).toBeVisible();
...
test.describe('Registration Flow', () => {
  test('should display registration form', async ({ page }) => {
    ...
    const registerLink = page.getByRole('link', { name: /inscription|register|s'inscrire|créer un compte/i }).first();

    if (await registerLink.isVisible()) {
      await registerLink.click();
      await expect(page.getByRole('heading', { name: /inscription|register|s'inscrire/i })).toBeVisible();
    }
  });
});
