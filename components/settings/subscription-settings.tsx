...
export function SubscriptionSettings({ initialData, onSuccess, onError }: SubscriptionSettingsProps) {
            ...
            render={({ field }) => (
              <div className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  {Object.entries(subscriptionPlans).map(([planKey, planData]) => {
                    const isSelected = field.value === planKey;
                    ...
                    return (
                      <div
                        ...
                        onClick={() => field.onChange(planKey)}
                      >
                        <div className="p-6">
                          {/* Plan Header */}
                          <div className="flex items-center justify-between mb-4">
                            <div className={`p-2 rounded-lg bg-gradient-to-r ${getPlanColor(plan)} text-white`}>
                              {getPlanIcon(plan)}
                            </div>
                            {isSelected && (
          ...
          {selectedPlan && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-green-800">
                <strong>✅ Plan actuel :</strong> {subscriptionPlans[selectedPlan].label}
                <br />
                {selectedPlan !== 'free' && (
          ...
          {selectedPlan !== 'free' && isDirty && (
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                ...
                <br />
                En changeant votre plan, vous serez redirigé vers notre système de paiement sécurisé.
                Le changement prendra effet immédiatement après confirmation.
              </p>
            </div>
          )}
              ...
              >
                {isLoading ? (
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    {selectedPlan === 'free' ? 'Passer au plan gratuit' : 'Mettre à jour l\'abonnement'}
                  </span>
                )}
              </Button>
            </div>
