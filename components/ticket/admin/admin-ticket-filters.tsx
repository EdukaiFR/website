...
import type { TicketConfigValue, AdminUser } from "@/lib/types/ticket";
import {
  STATUS_LABELS,
  TYPE_LABELS,
  CATEGORY_LABELS,
...
import type { AdminTicketFilterValues } from "@/hooks/useAdminTickets";
import { DEFAULT_ADMIN_FILTERS } from "@/hooks/useAdminTickets";
...
/**
 * Admin filter bar with search, status, type, category, urgency, and assigned-to comboboxes.
 */
export function AdminTicketFilters({
  ...
  const updateFilter = useCallback(
    (key: keyof AdminTicketFilterValues, value: string) => {
  ...
  const hasActiveFilters =
    filters.status !== DEFAULT_ADMIN_FILTERS.status ||
    filters.type !== DEFAULT_ADMIN_FILTERS.type ||
    filters.category !== DEFAULT_ADMIN_FILTERS.category ||
    filters.urgency !== DEFAULT_ADMIN_FILTERS.urgency ||
    filters.assignedTo !== DEFAULT_ADMIN_FILTERS.assignedTo ||
    filters.search !== DEFAULT_ADMIN_FILTERS.search;
  ...
  const assigneeOptions = adminUsers.map((u) => ({
    key: u._id,
